...
class TraceTable():
    ...
    def toggleGrid(self):
        ...
        plotItem = self.graph.plot.getPlotItem()
        if self.showGrid.isChecked():
            plotItem.getAxis('left').setGrid(255)
            plotItem.getAxis('bottom').setGrid(255)
        else:
            plotItem.getAxis('left').setGrid(False)
            plotItem.getAxis('bottom').setGrid(False)

    def toggleLegend(self):
        if self.showLegend.isChecked():
            self.legend = pqg.LegendItem()
            for trace in self.graph.traces.itervalues():
                self.legend.addItem(trace, trace.NAME)
            self.legend.setParentItem(self.graph.plot.getPlotItem())
        elif self.legend != None:
            vb = self.graph.plot.getPlotItem().getViewBox()
